...
namespace scli
{
    OneBot::OneBot()
    {
        ...
        if (m_Curl == nullptr)
            Console::error("Failed init curl");
        ...
        std::string token = fmt::format("Authorization: Bearer {0}", "1234567890");
            ...
            if (message_type == "group")
            {
                ...
                for (nlohmann::json message: msg["message"])
                {
                    if (message["type"] == "text")
                        recvMsg.msg.push_back(SingleMsg{ SingleMsgType::Text, message["data"]["text"].get<std::string>() });

                    if (message["type"] == "image")
                        recvMsg.msg.push_back(SingleMsg{ SingleMsgType::Image, message["data"]["file"].get<std::string>() });

                    if (message["type"] == "reply")
                        recvMsg.msg.push_back(SingleMsg{ SingleMsgType::Reply, message["data"]["id"].get<std::string>() });

                    if (message["type"] == "at")
                        recvMsg.msg.push_back(SingleMsg{ SingleMsgType::At, message["data"]["qq"].get<std::string>() });
